				  ...
				  unsigned long unused, void *arg)
{
	...
	if (unlikely(load_based_syncs && (mnd->load < 0 || mnd->load > 100))) {
...
static int boost_mig_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (unlikely(delayed_work_pending(&s->boost_rem)))
...
static void do_input_boost(struct work_struct *work)
{
	...
	if (unlikely(delayed_work_pending(&input_boost_rem)))
