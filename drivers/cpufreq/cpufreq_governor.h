...
#define define_ratemin_node(gov)					\
...
/**
 * jiffy_sampling_rate() - return sampling_rate multiplied by 10 ticks.
 */
 ...
 * @rate_mult: multiplicator to be used on the delay.
 */
static inline u32 align_delay(u32 sampling_rate, u32 rate_mult)
{
	int delay = usecs_to_jiffies(sampling_rate * rate_mult);
 ...
 *
 * Frequency is aligned to a higher one. If current frequency of a policy is
 * already maxed out, function will return early.
 */
