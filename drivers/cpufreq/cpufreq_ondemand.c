					   ...
					   u32 *max_load_ptr)
{
	...
	for_each_cpu(cpu, policy->cpus) {
		...
		/*
		 * Try to get average CPU frequency.
		 * Fallback to current frequency upon failure.
		 */
		freq_avg = __cpufreq_driver_getavg(policy, cpu);
		if (freq_avg <= 0)
			freq_avg = policy->cur;

		/* Ondemand uses averaged 'load' values during calculations */
	...
	/* Though, rough 'load' value is required for an exact algorithm */
...
static inline u32 get_policy_max_load_other_cpu(struct cpufreq_policy *policy)
{
	...
	for_each_online_cpu(cpu) {
		...
		/* Count a maximum 'load' value across all online cpus */
		max_load_other = max(max_load_other, j_dbs_info->max_load);
		...
		if (j_cdbs->cur_policy &&
			...
			max_load_other = od_tuners.up_threshold_any_cpu_load;
	}

	return max_load_other;
}
...
static void od_dbs_timer(struct work_struct *work)
{
	...
	int delay = align_delay(od_tuners.sampling_rate, dbs_info->rate_mult);
			       ...
			       unsigned long target_cpu, void *arg)
{
	 ...
	 */
	if (!atomic_cmpxchg(&dbs_info->being_woken, 0, 1)) {
...
static int od_sync_thread(void *data)
{
	...
	while (1) {
		...
		/*
		 * Use default values in case source cpu does not provide
		 * policy information.
		 */
		...
		/*
		 * Try to synchronize the frequencies of a source cpu and a
		 * destination one.
		 */
		if (policy->cur < src_freq) {
			...
			if (__cpufreq_driver_target(policy, src_freq,
				 ...
				 * affect the results of a load computation.
				 */
			...
			/* Put the work onto a global workqueue */
...
static void od_input_boost(struct work_struct *work)
{
	...
	/*
	 * od_exit() sets policy to NULL to disable input boosting
	 * before the unregistration of input handler happens.
	 */
...
static inline void update_sampling_rate(u32 new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (time_before(next_sampling, appointed_at))
			mod_delayed_work_on(cdbs->cpu, od_wq, &cdbs->work,
