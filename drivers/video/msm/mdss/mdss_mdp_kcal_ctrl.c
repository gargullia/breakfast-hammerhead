...
static inline void mdss_mdp_kcal_display_setup(void)
{
	...
	for (i = 0; i < mdata->nctl; i++) {
		ctl = mdata->ctl_off + i;
		/* We need to setup main (zero) MDSS panel only */
...
#define define_one_kcal_node(name, object, min, max, update_pa)		\
