...
int snd_ctrl_register(struct snd_ctrl_data *snd_data)
{
	...
	/*
	 * Incomplete control data can not be registered.  Also note that
	...
	if (likely(!find_ctrl_data(snd_data->name))) {
		...
		if (snd_ctrl_data_expected(snd_data) &&
		    snd_ctrl_data_fill_of(snd_data))
			pr_err("Failed to fill %s ctrl data\n", snd_data->name);
		 ...
		 *
		 * Also take expected codec into account.
		 */
		if (list_is_singular(&codec_list) ||
		    snd_ctrl_data_expected(snd_data)) {
			ctrl_data = snd_data;
...
#define get_line(fail, node, key, line) ({				   \
	int rc = of_property_read_u32(node, key, &stats->data.lines.line); \
	if (IS_ERR_VALUE(rc))						   \
