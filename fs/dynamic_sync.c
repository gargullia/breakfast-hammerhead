 ...
 * the risk of data loss on portable devices, which screens change their
 * states quite often. If LCD notifier is unused in this kernel, it will be
 * dropped anyway.
 */
#define DEF_LCD_NOTIFIER_IS_USED	1

/*
 * A delay before the file synchronization after turning off the display in ms.
 */
...
{
	/*
	 * Enable file synchronization instantly as userspace processes
	 * can use sync syscall to push the data after the panel turns off.
	 */
	fsync_enabled = true;
	...
	/*
	 * Do not push emergency synchronization immediately as the user
	 * can turn on the display within some seconds. Use a tunable value
	 * instead to delay the synchronization, so there won't be a high
...
static inline void dynamic_sync_resume(void)
{
	...
	if (delayed_work_pending(&force_sync_work))
		ret = cancel_delayed_work(&force_sync_work);
					  ...
					  const char *buf, size_t count)
{
	...
	if (dyn_sync.enabled && dyn_sync.lcd_notify_used)
		fsync_enabled = lcd_panel_suspended() > 0 ? true : false;
	else
		fsync_enabled = !val;

	/*
	 * Synchronize file system data right after the switch to ensure that
	 ...
	 * or a hardware failure lays on the user's shoulders.
	 */
	sys_sync();
...
static int __init dynamic_sync_init(void)
{
	...
	dfs_wq = alloc_workqueue("dynamic_sync_wq", WQ_HIGHPRI, 0);
	 ...
	 * expected to fail in case it is not used in kernel build.
	 */
